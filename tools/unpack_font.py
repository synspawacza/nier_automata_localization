...
parser.add_argument("font_file", help=".ftb file name")
parser.add_argument(
...
for c in ftb.chars:
    ...
    if args.skip_cjk and ord(c.char) >= 0x2E80 and ord(c.char) <= 0x9FFF:
        continue
    textures[c.texture_id].crop((c.u, c.v, c.u + c.width, c.v + c.height)).save(
        os.path.join(args.directory, "{:04x}.png".format(ord(c.char)))
    )
